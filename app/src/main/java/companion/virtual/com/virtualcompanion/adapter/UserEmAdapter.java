...
public class UserEmAdapter extends RecyclerView.Adapter<UserHolder> {

    private List<UserModel> userModelList = Collections.emptyList();
    private Activity activity;
    private LocationUserCallBack callBack;

    public UserEmAdapter(List<UserModel> userModelList, Activity activity, LocationUserCallBack callBack){
            ...
            public void onClick(View view) {
                try {
                    callBack.onClick(userModelList.get(positionFinal).getLatitude(),
                            userModelList.get(positionFinal).getLongitude(), userModelList.get(positionFinal));
                } catch (Exception ignore){}
            }
        });
    }
